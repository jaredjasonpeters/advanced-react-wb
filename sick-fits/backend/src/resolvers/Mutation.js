...
const mutations = {
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    if (!userId) {
      throw new Error("You must be signed in bruh");
    }
    ...
    //2. Check if that item is already in their cart
    //incrment by 1 if it is
    if (existingCartItem) {
    ...
    //3. If it is not, creat a fresh cart item for that user
